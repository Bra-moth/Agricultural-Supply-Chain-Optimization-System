...
# Configure databasefrom flask import Flask, render_template, request, redirect, url_for, flash, session
from werkzeug.security import generate_password_hash, check_password_hash
from datetime import datetime
from extensions import db
from models import User, Crop, Order
...
if __name__ == '__main__':
    app.run(debug=True)# No selected code provided, so I'll suggest a general improvement to the code

# Add a decorator to check if the user is logged in for routes that require authentication
...
# Apply the decorator to routes that require authentication
@app.route('/dashboard')
@login_required
def dashboard():
    user = User.query.get(session['user_id'])
...
def add_crop():
    ...
    return render_template('add_crop.html')from flask import Flask, render_template, request, redirect, url_for, flash, session
from werkzeug.security import generate_password_hash, check_password_hash
from datetime import datetime
from extensions import db
from models import User, Crop, Order
...
if __name__ == '__main__':
    app.run(debug=True)db.init_app(app)
